...
# Set AGENT_USAGE_STAT_DEBUG=1 to log which node was chosen.
set -e
...
if [ -z "$NODE_BIN" ]; then
  echo "agent-usage-stat: node not found on PATH or in known install locations" >&2
  exit 1
fi

[ -n "$AGENT_USAGE_STAT_DEBUG" ] && echo "agent-usage-stat: using $NODE_BIN" >&2

exec "$NODE_BIN" "$(dirname "$0")/agent-usage-stat.js" "$@"
